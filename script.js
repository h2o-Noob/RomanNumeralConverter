  ...
  if (count % 2 === 0) {
    ...
    let input = document.getElementById("decimal").value
      function changeExceptions(){
        return input = input.replace("iv", 'q').replace('ix', 'w').replace("xl", 'e').replace('xc', 'r').replace("cd", 't').replace('cm', 'y').replace("mV", 'u').replace('mX', 'o').replace("XL", 'p').replace('XC', 'k').replace("CD", 'j').replace('CM', 'h')}
